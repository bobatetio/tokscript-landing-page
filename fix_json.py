...
# Read the English file first to understand the structure
with open('locales/en.json', 'r', encoding='utf-8') as f:
    en_data = json.load(f)

# Create the translation manually with proper escaping
zh_data = {
    "meta": {
    ...
    "faq": {
        ...
        "q8": {
            ...
            "answer": "我们让下载您的 TikTok 视频字幕变得简单，耗时最少。要下载您的 TikTok 视频字幕，您只需要您希望获得字幕的视频链接。转到您希望下载的 TikTok 视频，复制视频链接，然后将其粘贴到上面的输入字段中。这将允许您立即下载 TikTok 视频字幕！您可以通过转到"分享"图标并点击"复制链接"来获取此链接。现在只需将其粘贴到我们的网站上，就完成了！"
        },
        "q9": {
            "title": "我如何下载 Instagram Reels 视频字幕？",
            "answer": "为了下载您的 Instagram Reels 视频字幕，您只需要视频的链接。转到您希望下载的 Instagram Reels 视频，复制视频链接，然后将其粘贴到我们网站上的输入字段中。这将允许您立即下载 Instagram Reels 视频字幕！您可以通过转到"分享"图标并点击"复制链接"来获取此链接。现在只需将其粘贴到我们的网站上，就完成了！"
        },
        "q10": {
            "title": "我如何下载 YouTube Shorts 视频字幕？",
            "answer": "很简单！您只需转到您希望下载的 YouTube Shorts 视频，复制视频链接，然后将其粘贴到我们网站上的输入字段中。这将允许您立即下载 YouTube Shorts 视频字幕！您可以通过转到"分享"图标并点击"复制链接"来获取此链接。现在只需将其粘贴到我们的网站上，就完成了！"
        },
        "q11": {
        ...
        "q12": {
            ...
            "answer": "我们可以从 TikTok、YouTube Shorts 和 IG Reels 的任何视频下载任何字幕，只要有字幕可下载。如果您的视频中没有"讲话"，或创作者已禁用平台内的自动转录，我们将无法下载它们。别担心，大多数视频都启用了视频字幕，这种情况很少发生。"
        },
        "q13": {
            "title": "我是否需要拥有 TikTok、YouTube 或 Instagram 账户才能下载字幕？",
            "answer": "绝对不需要！您不需要 TikTok、Instagram 或 YouTube 账户来下载您的视频字幕。您只需要您想下载字幕的视频链接。只需将您的链接粘贴到此页面顶部的输入字段中，点击"开始"，我们会处理其余的！"
        },
        "q14": {
...
# Write the translated data with proper formatting
with open('locales/zh.json', 'w', encoding='utf-8') as f:
    json.dump(zh_data, f, ensure_ascii=False, indent=2)

print("Created zh.json successfully!")
...
try:
    ...
    print(f"Total keys in zh.json: {sum(1 for _ in getAllKeys(loaded))}")
except Exception as e:
    print(f"JSON validation: FAILED - {e}")

def getAllKeys(obj, prefix=''):
    keys = []
    for x in obj.keys():
        if isinstance(obj[x], dict) and obj[x] is not None:
            keys.extend(getAllKeys(obj[x], prefix + x + '.'))
        else:
            keys.append(prefix + x)
    return keys
