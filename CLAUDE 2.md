...
You MUST use agent teams (TeamCreate) for ANY non-trivial task. Do not work solo when parallel execution would be faster or more thorough.

### Model Requirements

- All teammates MUST use Sonnet or Opus models. Never use Haiku for any agent team task.
- If a sub-agent or teammate defaults to Haiku, override it to Sonnet immediately.

### Team Planning Rules

- Before starting work, plan the team structure: identify distinct roles needed
- Create as many teammates as the task requires — don't under-staff
- Each teammate gets a clear, focused mission with no overlap
- Each teammate should leverage any applicable Claude Code skills, MCP tools, or slash commands available to them
- The lead orchestrates, delegates, and synthesizes — the lead does NOT do the implementation work itself
- Teammates should share findings, challenge each other, and coordinate independently
...
- The lead checks in on teammates and synthesizes their work
- If a teammate is blocked, the lead reassigns or spins up a new teammate
- When all teammates report back, the lead compiles a unified summary
