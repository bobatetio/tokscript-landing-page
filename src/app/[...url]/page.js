...
import { IoLockClosed, FaBrain, FaBolt, FaCloud, FaCopy, FaDownload, FaFireAlt, FaFolder, FaImage, FaLink, FaPenFancy, FaPlus, FaShare, FaChevronDown, TbBrandTiktok, TbBrandInstagram, TbBrandYoutube, HiLanguage } from "@/components/Icons";
...
export default function LandingPage({ params }) {
  ...
  useEffect(() => {
    const processUrlFromParams = async () => {
      if (
        unwrappedParams?.url &&
        Array.isArray(unwrappedParams.url) &&
        unwrappedParams.url.length > 0
      ) {
        // Reconstruct the full URL from the params array
        let rawUrl = unwrappedParams.url.join("/");

        // Decode the URL first (in case it's URL-encoded)
        let decodedUrl = decodeURIComponent(rawUrl);

        // Fix protocol - add missing slash after https: or http:
        // This handles browser URL normalization where https:// becomes https:/
        ...
        setTimeout(() => {
          ...
          if (!user || user == null) {
            // If not logged in, check if it's a premium platform
            // if (
            ...
            //   return;
            // }
            // For TikTok, process it directly
            fetchTikTokData(fullUrl);
          } else {
            // If logged in, redirect to the app with the URL
            window.location.href = `${process.env.NEXT_PUBLIC_FRONTEND_URL}/video/${fullUrl}`;
          }
        }, 500); // Small delay to ensure user state is loaded
      }
    };

    if (isClient) {
      processUrlFromParams();
    }
  }, [unwrappedParams, isClient, user]);
  ...
  const fetchTikTokData = async (urlToFetch = null) => {
    const videoUrl = urlToFetch || videoLink;
  ...
  // If URL doesn't look like a video link, show 404
  const urlPath = unwrappedParams?.url?.join("/") || "";
  const isVideoUrl =
    /(?:^|[./])(?:tiktok\.com|youtube\.com|youtu\.be|instagram\.com)(?:[/:]|$)/.test(
      urlPath,
    );
            ...
            <div className="inner-section">
              <div className="banner-content-wrapper">
                <button className="ai-btn">
                  <Image src={magicIcon} alt="magicIcon" />
                  AI Powered
                </button>
              ...
              <Image src={tiktok1Icon} alt="" className="tiktok-icon" />
              <Image src={youtube1Icon} alt="" className="youtube-icon" />
              <Image src={instagram1Icon} alt="" className="instagram-icon" />
            </div>
                  ...
                  <div className="col-12 col-lg-6 order-1 order-lg-2 mb-4 mb-lg-0">
                    <div className="content-main-wrapper">
                      ...
                      <p className="bulk-text-div">
                        ...
                        details, video sources, and durations. Instantly
                        re-download transcripts in TXT, XML, or PDF formats, and
                        organize your entire video library with custom bookmark
                        folders. Effortlessly share, bulk export, or review
                        transcripts, collections, and playlists for ultimate
                        video transcript organization and workflow management.
                      </p>
                      <ul>
                      ...
                      <div className="ai-listing">
                        <ul>
                          <li>
                            Viral Hook Generator:
                            <p>
                              Generate scroll-stopping, proven hooks in
                              seconds. No more guessing or losing viewers in the
                              first five seconds.
                            </p>
                          </li>
                          <li>
            ...
            <div className="inner-section">
              <div className="content">
                ...
                <p>
                  ...
                  <br />
                  Download any video's captions, transcripts, and words for your
                  TikTok, Instagram Reels, and Youtube Shorts videos in seconds.
                  Add your video link, hit start, and instantly get any TikTok,
                  Reels, or Shorts video script within seconds. Perfect for UGC
                  creators, Media Buyers, Ads Experts, Creators, and Influencers
                  who need help coming up with ideas or understanding what
                  videos
                </p>
                <p>
                  Easily and instantly download any video transcripts from your
                  favorite TikTok, Youtube Shorts, and Instagram Reels videos
                  instantly!
                </p>
              </div>
              <div className="content">
